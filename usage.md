# 基本操作

この章では、ホロヨエルの表情や身体ギミックなど、主なインタラクション操作について解説します。

---

## Expression Menu（表情・ギミック操作）

ホロヨエルのメニューは以下の用になっています。以下順番に解説します。

![Expression1](Manual_Pict/Expression1.png)

---

## BodyMenu

BodyMenuでは以下の項目を設定できます：

- **Bsize_Step1**：胸のサイズ調整。デフォルトは45%。
- **Bsize_Step2**：胸のサイズ調整。デフォルトは0%。
- **Chubby_Belly**：お腹がちょっと太ります
- **Chubby_Spine**：胴体がちょっと太ります
- **Chubby_Back**：背中がちょっと太ります
- **ClawToggle**：爪を出し入れします。
- **ClawAuto**：ONの間、両手を顔の前に構えて Fist -> Openとジェスチャーすることによりメニューを開かずに爪の出し入れができます。  

![Expression2](Manual_Pict/Expression2.png)

---

## SakeMenu

SakeMenuではお酒のボトルの出し入れができます。

![Expression3](Manual_Pict/Expression3.png)

酒は一定以上傾けると中身がこぼれます。





**ここに酒をこぼしているいい感じの画像**



------

## Halo setting, Cloth Toggle menu

![Expression5](Manual_Pict\Expression5.png)



---

## FaceEmo（表情セット切り替え）

FaceEmoでは以下の4種類の表情セットから1つを選んで使用できます。表情は基本的に右手で操作しますが、FaceEmoの設定から左右を逆にすることも可能です。：

![Expression4](Manual_Pict/Expression4.png)

| ![Emotion_1](Manual_Pict\Emotion_1.png) |
| :-------------------------------------: |
| ![Emotion_2](Manual_Pict\Emotion_2.png) |
| ![Emotion_3](Manual_Pict\Emotion_3.png) |
|                                         |



---

## FaceEmo の操作方法について

FaceEmoの基本操作については、以下の公式ドキュメントをご参照ください：

👉 [FaceEmo 操作ガイド（Setting Menu）](https://suzuryg.github.io/face-emo/ja/docs/optional-functions/setting-menu/)

※ ダンスギミックの使用方法についても上記ページ内で解説されています。

※ 既知の不具合として、ダンスギミック中にBody AdjustギミックのBreast Sizeが正常に動作しないことを確認しています。Breast Size変更状態でダンスギミックを使う場合は後述の改変ガイドを参考にデフォルト体型を変更してください。